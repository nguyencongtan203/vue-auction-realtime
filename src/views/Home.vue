...
/* API base + images (giữ nguyên biến API) */
...
const totalPages = ref(1); // từ BE Page.totalPages
const pageContent = ref([]); // danh sách phiên của trang hiện tại

/* Fetch categories (ApiResponse) */
...
/* Fetch auctions (paged from BE, chuẩn ApiResponse) */
async function fetchAuctions() {
  try {
    ...
    // Trang chủ hiển thị phiên "sắp diễn ra" → NOT_STARTED
    params.append("status", "NOT_STARTED");
    params.append("page", page.value - 1); // BE 0-based
    params.append("size", pageSize.value);
    params.append("sort", "thoigianbd,asc");

    const url = `${API}/auctions/find-by-single-status?${params.toString()}`;
    const res = await axios.get(url);

    if (res.data?.code === 200 && res.data.result) {
      const pg = res.data.result; // Spring Page
      ...
      pageContent.value = (pg.content || []).map((it) => {
        ...
        const item = {
          ...
          // BE có thể trả cả danhMuc trong sanPham
      ...
      // Thống kê tổng số phiên phù hợp
...
/* Filtering trên content của trang hiện tại */
const filteredPageContent = computed(() => {
  const q = search.value.toLowerCase().trim();
  return pageContent.value.filter((it) => {
...
/* Reset về trang 1 khi đổi filter text/category */
watch(search, () => {
  page.value = 1;
  // Lọc client trên content trang hiện tại
});
watch(selectedCate, () => {
  page.value = 1;
});
