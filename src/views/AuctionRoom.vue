<template>
  <div class="max-w-[1400px] mx-auto px-4 lg:px-6 mt-5 relative">
    ...
    <div class="flex gap-3 mt-4 mx-auto max-w-[1200px]">
      <button
        class="tab-btn"
        :class="statusFilter === 'upcoming' ? 'active' : ''"
...
/* ====== Fetch dữ liệu từ server (phân trang) ====== */
async function fetchAuctions() {
  try {
    ...
    const url = `${API}/auctions/find-by-status?${params.toString()}`;
...
/* ====== Lọc thêm (client trên content trang hiện tại) ====== */
const filteredPageContent = computed(() => {
  ...
  return pageContent.value.filter((it) => {
    ...
    const matchRegion = !selectedRegion.value || it.region === selectedRegion.value.matp;
    const matchPrice =
      (!priceRange.min || it.priceStartVnd >= priceRange.min) &&
      (!priceRange.max || it.priceStartVnd <= priceRange.max);
    const matchDate = it.startAt >= from && it.startAt <= to;
    return matchSearch && matchCate && matchRegion && matchPrice && matchDate;
  });
});
...
/* ====== Watch search để refilter (không cần refetch nếu chỉ lọc client) ====== */
watch(search, () => {
  // chỉ lọc trên trang hiện tại, không refetch
});
